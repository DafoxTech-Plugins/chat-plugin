...
exports.subscribe = async (device) => {
  ...
  interval = setInterval(() => {
    ...
    var seconds = s.filter(i => ['subscription', 'time', 'time_or_data'].includes(i.type)).reduce((t, _s, i) => {
    ...
    var megabytes = s.filter(i => ['time_or_data', 'data'].includes(i.type)).reduce((t, _s, i) => {
